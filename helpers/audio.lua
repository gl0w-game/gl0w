local audio = {}
    ...
    local ch3 = denver.get({waveform='square', frequency="C0"})
    local ch4 = denver.get({waveform='square', frequency="C0"})
    ch1:setLooping(true)
    ch2:setLooping(true)
    ch3:setLooping(true)
    ch4:setLooping(true)
    ...
    local jump = love.audio.newSource("assets/samples/jump.wav","static")
    ...
    audio.setPlaying = function(pstatus)
        playing = pstatus
    end

    audio.setTempo = function(tempo)
        bpm = tempo
    end

    audio.setMelody = function(melody)
        notes2 = melodies[melody+1]
    end

    audio.run = function(dt)
    	if playing == true then
    		...
    		if posnext >= 1 then
                ...
                if notes1[bar][sixteenth] == nil then
                    ...
                    ch3:setPitch( 0.0038*440*math.pow(2,notes1[bar][sixteenth]/12) )
                end
                ...
                if notes2[bar][sixteenth] == nil then
                    ...
                    ch4:setVolume( 0.5 )
                    ch4:setPitch( 0.0038*440*math.pow(2,notes2[bar][sixteenth]/12) )
                end
